...
function Home({
  ...
  /* ---------- Redirect handling (password reset → login) ---------- */
  ...
  return (
    <>
                  ...
                  {servicesData
                    .filter((service) => {
                      if (/^\d+$/.test(searchInput)) {
                        return service.price <= Number(searchInput);
                      }
                      return (
                        service.title.toLowerCase().includes(searchInput.toLowerCase()) ||
                        service.type.toLowerCase().includes(searchInput.toLowerCase())
                      );
                    })
                    .map((service) => (
                      <Link
                        /* ✔ fixed path syntax here */
      ...
      <ShowcaseCarousel showcaseItems={trendingServices} divID="trendingPlacesDivID" />

      {advtData && <AdvtBanner image={advtData.image} link={advtData.link} />}

      <h3 className="home__showcaseHeading home__headingSpecial">Special Packages</h3>
